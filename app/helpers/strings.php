...
/**
 * get section in Vendor page
 *
 * Example output: 'shop', 'profile'.
 *
 * @return string
 */
if (!function_exists('getVendorsPlace')) {
    function getVendorsPlace()
    {
        $request = Request::capture();
        $segments = $request->segments();
        $isSlug = preg_match('/^[a-z0-9]+(?:-[a-z0-9]+)*$/i', end($segments));
        $section = $isSlug ? prev($segments) : end($segments);
...
/**
 * replace keywords onto "Cool Orca" recursive loop for prevent duplicate
 *
 * @return string
 */
if (!function_exists('replacerBrandNames')) {
    function replacerBrandNames( $searched_text )
    {
        ...
        foreach ( $replace_words as $replace_word ){
            $substring = stristr($searched_text, $replace_word);

            if ($substring !== false) {
                if ( $first_replace == false ){
                    $searched_text = str_replace( $replace_word, 'Cool Orca', $searched_text );
                    $first_replace = true;
                } else {
                    $searched_text = str_replace( $replace_word . ' ', '', $searched_text );
                }
            }
 ...
 * @return string
 */
if (!function_exists('removerBrandNames')) {
    function removerBrandNames( $searched_text )
    {
        ...
        foreach ($remove_words as $word) {
            $searched_text = str_ireplace($word, '', $searched_text);
        }

        $searched_text = preg_replace('/\s+/', ' ', $searched_text);
...
if (!function_exists('removeSymbols')) {
    function removeSymbols( string|array $symbols, $value ): string
    {
        return str_ireplace($symbols, "", $value);
